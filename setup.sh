...
if ! which -s cqlsh; then
    echo You need to have cassandra installed
    exit 1
fi

if ! which -s gsed; then
    echo "You need to have gnu sed (gsed) installed."
    exit 1
fi
...
[ ! -d $TODO_APP_DIR/.git ] && git clone https://github.com/hkroger/todo.git $TODO_APP_DIR
